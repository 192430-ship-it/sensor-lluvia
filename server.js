...
const TELEGRAM_TOKEN = '8863488267:AAFrt9EFe7MfrzCvF0XNxzr_s-pHS77RU40';
const TELEGRAM_CHAT_ID = '7144279059';
let ultimoEstado = 'SECO';

function enviarAlarma(estado, valor) {
  const https = require('https');
  const emoji = estado === 'LLUVIA' ? '(lluvia)' : '(seco)';
  const texto = 'ALERTA SENSOR DE LLUVIA\nEstado: ' + estado + '\nValor: ' + valor + '\nUmbral: 2000\nHora: ' + new Date().toLocaleString('es-PE', {timeZone: 'America/Lima'});
  const body = JSON.stringify({ chat_id: TELEGRAM_CHAT_ID, text: texto });
  const options = {
...
app.get('/datos', async (req, res) => {
  try {
    ...
    if (items && items.length > 0) {
      const ul = items[0];
      const estado = ul.payload ? ul.payload.estado : ul.estado;
      const valor = ul.payload ? ul.payload.valor_analogico : ul.valor_analogico;
      if (estado !== ultimoEstado) {
        ultimoEstado = estado;
        enviarAlarma(estado, valor);
      }
    }
    res.json(items);
  } catch (err) {
